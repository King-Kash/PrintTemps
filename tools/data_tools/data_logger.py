...
try:
    resp = requests.get(f"http://{DUET_IP}/rr_connect?password=", timeout=0.5)
    data = resp.json()
except Exception as e:
    print("FAILED TO CONNECT WITH DUET BOARD.")
    print("Error:", e)

# Check for a successful reply code
if data.get("err") != 0:
    print(f"Duet replied err={data.get('err')}, cannot continue.")

# Grab sessionKey if present, else default to 0
session_key = data.get("sessionKey", 0)
print("Connected to Duet Board. sessionKey =", session_key)
